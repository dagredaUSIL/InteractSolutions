...
-- DMS_PORTFOILS
select 
	dms_portfoils.dt_created as 'DT Created',
	dms_portfoils.dt_read as 'DT Read',
	dms_portfoils.dt_confirmation as 'DT Confirmation',
	case dms_portfoils.state
		when 1 then 'No accedido'
		when 2 then 'Accedido'
		when 3 then 'Confirmado'
	end as 'State',
	dms_portfoils.ref_owner as 'ID user',
	cmn_users.name as 'User',
	dms_portfoils.ref_document as 'ID document',
	dms_documents.code as 'Code',
	dms_documents.name as 'Document',
	dms_documents.version as 'Version'

from dms_portfoils
join cmn_users on (dms_portfoils.ref_owner = cmn_users.id)
join dms_documents on (dms_portfoils.ref_document = dms_documents.id)

where 
	dms_portfoils.type = 5
